-- =============================================================
-- Hocta Conecta — Setup V2 (delta sobre o V1)
-- Rode este script no SQL Editor depois do SETUP_SUPABASE.sql.
-- Idempotente: pode ser rodado várias vezes sem quebrar.
-- =============================================================

-- ============ 1. Municípios (catálogo IBGE) ============
...
create index if not exists idx_municipios_nome on public.municipios(lower(nome));
...
-- ============ 3. Vínculo prestador <-> especialidade (N:N) ============
-- Mantém prestador_especialidades antigo mas adiciona FK opcional ao catálogo.
alter table public.prestador_especialidades
  add column if not exists especialidade_id integer references public.especialidades(id) on delete set null;

-- ============ 4. Cobertura geográfica do prestador ============
create table if not exists public.prestador_municipios (
  prestador_id      integer not null references public.prestadores(id) on delete cascade,
  municipio_codigo  bigint not null references public.municipios(codigo_ibge),
  primary key (prestador_id, municipio_codigo)
);

-- ============ 5. Municípios do projeto ============
create table if not exists public.projeto_municipios (
  ...
  municipio_codigo  bigint not null references public.municipios(codigo_ibge),
  primary key (projeto_id, municipio_codigo)
);

-- ============ 6. Metas do projeto ============
create table if not exists public.metas_projeto (
  id                serial primary key,
  projeto_id        integer not null references public.projetos(id) on delete cascade,
  especialidade_id  integer references public.especialidades(id) on delete cascade,
  municipio_codigo  bigint  references public.municipios(codigo_ibge),
  quantidade_meta   integer not null default 0,
  observacao        text,
  criado_em         timestamptz not null default now()
);
create index if not exists idx_metas_projeto on public.metas_projeto(projeto_id);

-- ============ 7. Benchmark ============
create table if not exists public.benchmarks (
  id                  serial primary key,
  origem              text not null check (origem in ('mercado','projeto_anterior')),
  especialidade_id    integer references public.especialidades(id) on delete set null,
  especialidade_texto text,
  municipio_codigo    bigint references public.municipios(codigo_ibge),
  uf                  char(2),
  quantidade          integer not null default 0,
  referencia_texto    text,
  projeto_origem_id   integer references public.projetos(id) on delete set null,
  criado_em           timestamptz not null default now()
);
create index if not exists idx_benchmark_esp on public.benchmarks(especialidade_id);

-- ============ 8. Interações de prospecção ============
create table if not exists public.prospeccao_interacoes (
  id             serial primary key,
  prospeccao_id  integer not null references public.prospeccoes(id) on delete cascade,
  tipo           text not null check (tipo in ('telefone','email','whatsapp','visita','outro')),
  data           timestamptz not null default now(),
  observacao     text,
  autor_id       uuid references auth.users(id) on delete set null,
  criado_em      timestamptz not null default now()
);
create index if not exists idx_interacoes_prosp on public.prospeccao_interacoes(prospeccao_id);

-- ============ 9. Prospecção -> rastreamento de conversão ============
-- prestador_id já existe e é NOT NULL; mantemos compat.
-- Adicionamos flag de "convertido" pra distinguir credenciamento via funil.
...
grant select, insert, update, delete on public.prestador_municipios to authenticated;
grant select, insert, update, delete on public.projeto_municipios to authenticated;
grant select, insert, update, delete on public.metas_projeto to authenticated;
grant select, insert, update, delete on public.benchmarks to authenticated;
grant select, insert, update, delete on public.prospeccao_interacoes to authenticated;
grant usage, select on all sequences in schema public to authenticated;

grant all on public.municipios to service_role;
grant all on public.especialidades to service_role;
grant all on public.prestador_municipios to service_role;
grant all on public.projeto_municipios to service_role;
grant all on public.metas_projeto to service_role;
grant all on public.benchmarks to service_role;
grant all on public.prospeccao_interacoes to service_role;

-- ============ RLS ============
alter table public.municipios enable row level security;
alter table public.especialidades enable row level security;
alter table public.prestador_municipios enable row level security;
alter table public.projeto_municipios enable row level security;
alter table public.metas_projeto enable row level security;
...
create policy "municipios_read" on public.municipios for select to authenticated using (true);

drop policy if exists "especialidades_all" on public.especialidades;
create policy "especialidades_all" on public.especialidades for all to authenticated using (true) with check (true);

drop policy if exists "prestador_municipios_all" on public.prestador_municipios;
create policy "prestador_municipios_all" on public.prestador_municipios for all to authenticated using (true) with check (true);

drop policy if exists "projeto_municipios_all" on public.projeto_municipios;
create policy "projeto_municipios_all" on public.projeto_municipios for all to authenticated using (true) with check (true);

drop policy if exists "metas_projeto_all" on public.metas_projeto;
create policy "metas_projeto_all" on public.metas_projeto for all to authenticated using (true) with check (true);

drop policy if exists "benchmarks_all" on public.benchmarks;
create policy "benchmarks_all" on public.benchmarks for all to authenticated using (true) with check (true);

drop policy if exists "prospeccao_interacoes_all" on public.prospeccao_interacoes;
create policy "prospeccao_interacoes_all" on public.prospeccao_interacoes for all to authenticated using (true) with check (true);

-- ============ Seed: especialidades médicas comuns ============
insert into public.especialidades (nome) values
  ('Cardiologia'),('Clínica Médica'),('Dermatologia'),('Endocrinologia'),
  ('Gastroenterologia'),('Geriatria'),('Ginecologia e Obstetrícia'),('Hematologia'),
  ('Infectologia'),('Mastologia'),('Nefrologia'),('Neurologia'),
  ('Nutrição'),('Oftalmologia'),('Oncologia'),('Ortopedia'),
  ('Otorrinolaringologia'),('Pediatria'),('Pneumologia'),('Psicologia'),
  ('Psiquiatria'),('Radiologia'),('Reumatologia'),('Urologia'),
  ('Fisioterapia'),('Fonoaudiologia'),('Odontologia'),('Cirurgia Geral'),
  ('Cirurgia Vascular'),('Anestesiologia')
on conflict (nome) do nothing;

-- ============ Seed: Municípios (IBGE oficial) ============
insert into public.municipios (codigo_ibge,nome,uf) values
